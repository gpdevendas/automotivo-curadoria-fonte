...
test('completa edição parcial até 10, preserva original e renderiza categorias repetidas', async t => {
  const a = ambiente(t);
  ...
  assert.equal((content.match(/^Fato:/gm) || []).length, 10);
  assert.ok(content.includes('Fato original.'));
  assert.ok(content.includes('10 notícias selecionadas'));
  await a.main();
  ...
  assert.equal(Object.values(dados[0].categorias).flat().length, 10);
  assert.equal(dados[0].categorias['Compra e venda / mercado'].length, 3);
...
test('pesquisas extras completam dez quando as cinco primeiras não rendem', async t => {
  const a = ambiente(t);
  let buscas = 0;
  a.context.fetch = async (url, opts) => {
    if (url.endsWith('/search')) {
      ...
      assert.ok(body.query.includes('site:'));
      return resposta({ web: buscas <= 5 ? [] : [1, 2].map(n => ({
        url: `https://exemplo.com/${buscas}-${n}`, date: '2020-01-01', title: `Nova ${buscas}-${n}`,
      })) });
    }
    return resposta({ summary: 'Fato novo.', metadata: { 'article:published_time': a.hoje } });
  };
  await a.main();
  const digest = fs.readFileSync(path.join(a.root, 'curadoria', `${a.hoje}.md`), 'utf8');
  assert.equal(buscas, 10);
  assert.equal((digest.match(/^Fato:/gm) || []).length, 10);
});
